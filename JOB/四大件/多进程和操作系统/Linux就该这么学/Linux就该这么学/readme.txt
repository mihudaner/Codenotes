...
EXPANSION
   ...
   Pathname Expansion
       ...
       gard  to the case of alphabetic characters.  Note that when using range
       ...
       name  or  immediately following a slash must be matched explicitly, un‐
       less the shell option dotglob is set.  The filenames ``.''  and  ``..''
       must  always  be  matched explicitly, even if dotglob is set.  In other
       cases, the ``.''  character is not treated specially.  When matching  a
       ...
       by  a special pattern character as described below under Pattern Match‐
       ing.  See the description of shopt below under SHELL  BUILTIN  COMMANDS
       for  a  description  of the nocaseglob, nullglob, failglob, and dotglob
       shell options.
