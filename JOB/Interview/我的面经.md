...
为什么要把编译运行和oj_server分开？（因为编译运行需要的时间长，便于分离部署等）

如何实现的负载均衡？
在开发过程中，你是如何调试的？（gdb，vscode插件）

项目使用的是什么协议（http或者tcp），

tcp和udp有什么不同？（特点）

tcp是如何保证可靠性的（校验和，确认应答，确认序号，超时重传，流量控制，拥塞控制），tcp如何保证发送顺序？（确认序号）

 一个编译服务器一次只编译运行一个请求是不是太浪费了，能不能优化一下？
