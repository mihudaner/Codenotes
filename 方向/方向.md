...
>
> VirPNet 包括两个阶段：
> 虚拟点生成阶段和 3D 物体检测阶段。阶段。 首先，虚拟点生成阶段
> 生成虚拟点云来完成稀疏物体的点云。这得益于图像和点云之间的互补信息。
> 图像和点云之间的互补信息。 与之前的不同的是，我们利用像素和点云之间的几何关系
> 点云之间的几何关系，将三维空间的距离回归简化为一维空间的距离回归。
>
> 我们利用几何关系创建一个虚拟平面，将三维点云回归简化为
> 一维距离回归。
>
> - 我们提出了一种新颖的虚拟点生成网络
> 我们提出了一种新颖的虚拟点生成网络，用来自图像和激光雷达融合输入的稀疏点云来增强物体的点云。
> 我们提出了一种新颖的虚拟点生成网络，利用来自图像和激光雷达融合输入的稀疏点云来增强物体的点云。
> - 我们将虚拟点生成网络集成到现成的基于点的三维骨干网中，以提高三维物体检测能力。
> 检测。
> - 我们在 KITTI 三维物体检测数据上进行了一系列对比和消融实验
> 对 KITTI 三维物体检测数据集和 nuScenes
> 数据集进行了一组对比和消融实验，以解释 VirPNet 的优越性。

> ![image-20241114184052640](E:\codenotes\方向\img\image-20241114184052640.png)
>
> 虚拟点生成阶段首先将点云分成关键点、前景点、背景点和预定义的虚拟点。
> 点。 其次，将这四类点保存为图像格式，并与原始图像串联，得到融合输入。 然后，将融合输入
> 输入二维编码器-解码器网络，生成虚拟点云。 三维检测阶段是一个以点为基础的三维检测框架。
> 原始点云和第(i)阶段生成的虚拟点云作为输入，生成最终的三维检测结果

> ![image-20241114185050629](E:\codenotes\方向\img\image-20241114185050629.png)
>
> 如图 3 所示，三维空间中有一个物体（圆柱体）。
> 空间。 
>
> - S1 是图像平面
>
> - S2 是虚拟平面通过点 P0 并与 S1 平行。 
>
> - 激光雷达检测一组稀疏的点云P0(x0;y0;z0)。
> - 点P是理论那个像素在物体上的真实点
> - P'是预定义虚拟点
>
> 
>
> 预定义的点 是点P 在 S2 平面上的投影点。
>
> 
>
>  摄像头捕捉一组密集像素，例如Q0(u; v)。
>
>  在这里，P0 和 Q0 代表不同模式下的同一物体点。
> 在不同的模式下。 P0 和 Q0 的对应关系
> 可通过以下几何变换实现
>
> 给定一组像素偏移（du; dv），就能唯一确定一个新像素 Q(u + du; v + dv)，如图所示。
> du; v + dv）可以唯一确定，如图 3 所示。
>
> 
>
> 假设该像素代表三维空间中的一个点，该点是激光雷达传感器所扫描的三维空间中的一个点。 虽然其对应的三维
> 点云（P(x; y; z)）无法确定
>
> 但 P 必须出现在线段 OQ 的延长线上。必须出现在线段 OQ 的延长线上，其中 O 是相机坐标的原点。相机坐标的原点。 此时，虚拟点 P 可能是点（紫色点）。OQ。 为了大致确定虚拟点 P 的位置，可以用一个虚拟平面
> S2（图 3 中的蓝色矩形）与图像平面 S1 平行。进行虚拟。 在虚拟平面 S2 的帮助下，虚拟点被唯一地确定下来。点，例如 P0（蓝点）。 这是因为
> **因为线段 OQ 的延长线和虚拟平面 S2**
> **只能相交于一点。 这个点就是 P0**，也是
> 预定义的虚拟点。 直观地说，如果预定义的虚拟点
> 点 P' 是一个物体的前景点，那么它就会非常接近真实点云 P（橙色点）非常接近，并且 P 必须出现在线段 OP0 或其上。必须出现在线段 OP' 或其延长线上。 否则 P0 为背景点，P0 可能是任何虚假点（紫色点）。
>
> ```
> 	k_x, k_y, k_z = k_pt_3d_rect[0] 
>         dx = k_z / f * du
>         dy = k_z / f * dv
>       
> 
...
> (1) 哪些点
> 应作为关键点采样？ 虚拟点生成
> 网络会根据选定的关键点自动生成多个虚拟点。 考虑到以下事实
> 过多的关键点可能会在如此小的区域内产生大量的虚拟点。
> 这对检测物体几乎没有帮助。 这不仅不利于检测物体，还会增加神经网络的工作量。
> 同时也会增加神经网络的工作量。 因此，我们提供了一种二维网格
...
```mermaid
flowchart LR
   KITTI -- prepare_data.py --> pickle

```

```mermaid
flowchart LR
   KITTI --  split_data_with_instance.py --> 筛选出含行人的文件 --create_instance_npz.py --> instance_nps&instance_pts_nps -- split_train_val.py -->instance_dataset
```

```mermaid
flowchart LR
instance_nps&instance_pts_nps  --create_label.py-->instance_label
```
...
>
> ```
> frustum_car_pedcyc_wimage_train/val/mini.pickle
> 内容：训练、验证、或小数据集的信息。
> 具体数据：
> 点云数据（可能裁剪到感兴趣区域，如目标的视锥体）。
> 每个样本的类别标签（Car, Cyclist, Pedestrian）。
> 投影矩阵、相机内参或点云坐标转换信息。
> 图像裁剪框（2D投影边界框）。
> 点云中每个点的掩码，标注哪些点属于目标。
> 
> train/val_Car/Cyclist/Pedestrian_box3d_mean_dimensions.pickle
> 内容：3D包围盒尺寸的统计信息。
> 具体数据：
> 不同类别物体的平均长、宽、高，用于模型初始化或正则化。
> ```
>
> ![image-20241128005913810](E:\codenotes\方向\img\image-20241128005913810.png)
>
> ![image-20241128005933505](E:\codenotes\方向\img\image-20241128005933505.png)
>
> ![image-20241128010001825](E:\codenotes\方向\img\image-20241128010001825.png)



> 2.  **split_data_with_instance.py**
>
> 筛选出含行人的文件



> 3. **create_instance_npz.py**
>
> 获得npz文件包含map信息，前景点，背景点，虚拟点等等信息
>
> 虚拟点没有真实的深度，那训练的标签是？
>
> 
...
> | 变量名称                      | 含义                                                 |
> | ----------------------------- | ---------------------------------------------------- |
> | pts_2d, pc_fov, fov_inds      | 2dbox视锥内点的像素坐标，3d坐标，是否是x<2.0         |
> | pts_2d3d_in_one_3dbbox,       | 视锥内的点在3dbox内的点的像素坐标                    |
> | pts_2d3d_out_one_3dbbox,      |                                                      |
> | pc_instance, pc_instance_neg, | 视锥内的点在3dbox内的点的3d坐标                      |
> | fov_inds_f, fov_inds_b        | 231个点   fov_inds_f 167个True   fov_inds_b 64个True |
> | pts_foreground_all            | 一帧的所有前景点的[uv,xyzr,grid]                     |
>
> ```
> 
>         # step 2-1: read 2d bbox & 3d bbox from label txt
>         # step 2-2: get pc in object fov
>         	获得每一个对象的2d坐标，3d坐标，是否是对象点
>         	pts_2d, pc_fov, fov_inds = get_lidar_in_image_fov   231个
> 
>          # step 2-3: calculate 2d/3d pts in one bbox
>          	calculate_pts_2d3d_in_3dbbox
>          	pts_2d3d_in_one_3dbbox, 在box内的点的2d,3d,grid坐标   64
>          	pts_2d3d_out_one_3dbbox,   167
>          	pc_instance, 
>          	pc_instance_neg, 
>          	fov_inds_f,  每个点在3d框外  231个
>          	fov_inds_b    内 231个
> 
>  # step 2-4 sample pts_foreground and pts_background
>  	一部分前景点变成的背景点每个目标最多sample_num = 500个点
> ```
>
> - project_points_velo2image
>
> ![image-20241128005434355](E:\codenotes\方向\img\image-20241128005434355.png)
>
> 
>
> 
...
> 处理了每一个grid内有哪些点
>
> 每个grid内计算bias_manhattan距离，选取最近的作为k点
>
> | 变量名称                             | 含义                                                      |
> | ------------------------------------ | --------------------------------------------------------- |
> | k_map, grid_ids_pos,                 | 像素哪些位置是k点       网格有点的grid                    |
> | k_pts_2d, k_pts_3d,                  | k点的2d,3d坐标                                            |
> | k_pts                                | [k_pt_2d, k_pt_3d, grid_id]   关键点                      |
> | pts_foreground                       | create_keypoint_map返回的是去除关键点的new_pts_foreground |
> | pts_background, masks = refine_b_pts | pts_background在网格内但是不在Mask内，   masks            |
> | f_map/b_map/fb_map                   | 前景点,背景点，前或者背map                                |
> |                                      |                                                           |
>
> ```
> create_keypoint_map
> refine_b_pts   # 原本背景点是所有不在Box内的点
> 
> 
> ```
>
> **step 5:** 5: create dudv_map/uv_map
>
> |                                                              |
> | ------------------------------------------------------------ |
> | `k_map`：关键点 (keypoints) 的占用地图。                     |
> | `f_map`：前景点 (foreground points) 的二值化地图。           |
> | `fb_map`：前景-背景点的对比地图。                            |
> | `k_pts`：关键点坐标列表。                                    |
> | `w, h`：输出地图的宽、高。                                   |
> | `N`：每个网格内生成的虚拟点 (VP) 数量。                      |
> | `M`：网格内允许的最大点数，超过则跳过处理。                  |
> | `GRID_SIZE`：网格大小。                                      |
> | uv_map, dudv_map, r_pts：      哪些点是虚拟点，距离中心点的偏移，虚拟点的uv和grid |
> | patch_free_ids  ：      是在一个网格里面10，10的局部相对坐标里，那些是空的像素点 |
> | M  配置文件里的网格内的点数超过则不补                        |
>
> ```
> # method 2: generate vp horizontally
> 选取像素点时候是通过一行以及间隔三个像素的行来选取的
>  uv_map[pt_k[0]+du, pt_k[1]+dv] = 1
>  dudv_map[pt_k[0] + du, pt_k[1] + dv, 0] = du
>  dudv_map[pt_k[0] + du, pt_k[1] + dv, 1] = dv
> ```
>
> ![image-20241128025241754](E:\codenotes\方向\img\image-20241128025241754.png)
>
> ![image-20241129154051867](E:\codenotes\方向\img\image-20241129154051867.png)





> 4. **create_instance_npz.py**
>
> 计算的应该是原理图没有添加误差的P`
>
> ![image-20241114185050629](E:\codenotes\方向\img\image-20241114185050629.png)
...
> 5. split_train_val.py
>
>    把instance_npz分成训练集和验证集合
>
> 6. create_label.py
>
> ```
> r_pts.append([[pt_k[0] + du, pt_k[1] + dv], grid_id])
> get_virtual_lidar_map得到的virtual_lidar_map是同k_pts的z   np.array([[round(k_x+dx, 6), round(k_y+dy, 3), round(k_z, 6)]])
> ```
>
> 当时需要虚拟点相对key值点的gt dl但是写在create_Myinstance_npz 里保存为的_label文件夹

> 6. dataset
>
> ```
> im_data = data = np.concatenate((img,         # 3
>                        k_map,       # 1 关键点 (keypoints) 的占用地图。
>                        k_pts_map,   # 4
>                        f_map,       # 1  前景点 (foreground points) 的二值化地图。
>                        f_pts_map,   # 4
>                        b_map,       # 1
>                        b_pts_map,   # 4
>                        uv_map,      # 1
...
<img src="E:\codenotes\方向\img\image-20241209163404078.png" alt="image-20241209163404078" style="zoom:50%;" />

<img src="E:\codenotes\方向\img\image-20241209163443549.png" alt="image-20241209163443549" style="zoom:50%;" />

![image-20241209164001123](E:\codenotes\方向\img\image-20241209164001123.png)

![image-20241209163934363](E:\codenotes\方向\img\image-20241209163934363.png)

### 计算同网格内的foreword点到Key点的du,dv和dl

```
create_uv_dudv_map里计算的是生成的虚拟点的du,dv
...
<img src="E:\codenotes\方向\img\image-20241213125921461.png" alt="image-20241213125921461" style="zoom:50%;" />
...
从dl和原始virtual_pc计算矫正的深度

那么get_predict_pc逆运算是？

save_kfb_pts_npz(im_name=im_name, k_pts=k_pts, f_pts=f_pts, b_pts=b_pts, r_pts=r_pts, save_folder=npz_pts_path)被覆盖了

```

```
uv_map, dudv_map, pts_regression = create_Mydudvdl
"""
uv_map :就是fore点的像素map
dudv_map:是fore点距离最近的k点的dudv（像素）,但是没有限制再一个gird里，原来好像是限制了虚拟点再一个grid
pts_regression :fore点的像素map,grid坐标
"""
```

```mermaid
flowchart LR
模型输出的reg_map  --filter_fuc-->滤除不合理值的pred_dl_map和virtual_lidar_map --filter_fuc-->pred_dl_map和virtual_lidar_map
pred_dl_map和virtual_lidar_map --->pc_virtual这就是最后要计算的不含dl的值--get_predict_pc+dl-->pc_pred
```

也就是说create_label生成的virtual_lidar_map其实不是训练gt而是纯计算没有加dl的深度

### 边缘溢出

![image-20241226150411219](E:\codenotes\方向\img\image-20241226150411219.png)

![image-20241226150422537](E:\codenotes\方向\img\image-20241226150422537.png)

![image-20241226154638975](E:\codenotes\方向\img\image-20241226154638975.png)

![image-20241226192100056](E:\codenotes\方向\img\image-20241226192100056.png)

![image-20241226192110538](E:\codenotes\方向\img\image-20241226192110538.png)

![image-20241226205734504](E:\codenotes\方向\img\image-20241226205734504.png)

![image-20241226230053152](E:\codenotes\方向\img\image-20241226230053152.png)

![image-20241227131627154](E:\codenotes\方向\img\image-20241227131627154.png)
